...
describe("openai-completions Anthropic cache_control", () => {
	...
	it("injects cache_control on system, last tool, and last conversation message for anthropic-id models", async () => {
		const tools: Tool[] = [
		...
		// One conversation breakpoint on the last message, matching upstream. Total
		// breakpoints (system + tool + 1 conv) stays within Anthropic's hard limit of 4.
		...
		expect(findCacheControl(assistantMsgs[assistantMsgs.length - 1].content)).toBe(false);
	});
